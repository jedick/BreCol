...
# Expand fit_setbert experiment output paths (same experiment list, features=setbert).
SETBERT_EXPERIMENT_OUTPUTS := $(shell $(PYTHON) -c "import yaml,pathlib; r=pathlib.Path('$(ROOT)'); cfg=yaml.safe_load((r/'experiments.yaml').read_text(encoding='utf-8')); sec=cfg.get('fit_classifier',{}); tpl=sec.get('results_json_template','results/{features}/{name}.json'); exps=sec.get('experiments',[]); print(' '.join(str((r/tpl.format(name=e['name'], features='setbert')).resolve()) for e in exps))")
...
# Select a single fit_setbert output path by 1-based EXPT index.
SETBERT_EXPERIMENT_OUTPUT := $(if $(filter-out 0,$(strip $(EXPT))),$(word $(EXPT),$(SETBERT_EXPERIMENT_OUTPUTS)),)
...
SETBERT_CSV := $(ROOT)/$(SETBERT_EMBEDDINGS_CSV)
...
help:
	...
	@echo ""
	@echo "  make setbert_embeddings"
	@echo "      FASTA -> run-level SetBERT [CLS] embeddings CSV at $(SETBERT_CSV)."
	...
	@echo "      <BLOG_REPO>/_posts/<POST_DATE>-<POST_SLUG>.md (date and slug fixed in"
	@echo "      helpers/publish_blog.py; last_modified_at is stamped with today's date);"
	@echo "      add PUSH=1 to commit and push."
	@echo ""
...
ifeq ($(strip $(EXPT)),0)
fit_setbert: $(SETBERT_EXPERIMENT_OUTPUTS)
	@echo "Up to date: all fit_setbert experiments"
else ifneq ($(strip $(EXPT)),)
fit_setbert: $(SETBERT_EXPERIMENT_OUTPUT)
	@if test -z "$(SETBERT_EXPERIMENT_OUTPUT)"; then \
		echo "Invalid EXPT=$(EXPT). Use EXPT=0 for all, or EXPT=1..N from experiments.yaml."; \
		exit 2; \
	fi
else
fit_setbert: $(SETBERT_CSV) $(ROOT)/scripts/fit_classifier.py \
		$(ROOT)/defaults.yaml $(ROOT)/experiments.yaml
	cd "$(ROOT)" && $(PYTHON) scripts/fit_classifier.py --setbert --results-json $(EXPT_ARG)
endif

define setbert_experiment_rule
$(word $(1),$(SETBERT_EXPERIMENT_OUTPUTS)): $(SETBERT_CSV) \
		$(ROOT)/scripts/fit_classifier.py \
		$(ROOT)/defaults.yaml \
		$(ROOT)/experiments.yaml
	cd "$(ROOT)" && $(PYTHON) scripts/fit_classifier.py --setbert --expt $(1)
endef

$(foreach i,$(CLASSIFIER_EXPERIMENT_INDICES),$(eval $(call setbert_experiment_rule,$(i))))
...
$(MANUSCRIPT_JEKYLL): $(MANUSCRIPT_DEPS) $(MANUSCRIPT_FILTER_NUMFIG) $(MANUSCRIPT_FILTER_NUMTAB) $(MANUSCRIPT_FILTER_TABCAP) $(MANUSCRIPT_TABLE_HTML) | $(MANUSCRIPT_BUILD_DIR)
	cd "$(ROOT)/manuscript" && $(PANDOC) manuscript.md \
		...
		--citeproc \
		--lua-filter "$(MANUSCRIPT_FILTER_TABCAP)" \
		--lua-filter "$(MANUSCRIPT_FILTER_NUMFIG)" \
		--to=gfm --wrap=preserve \
		-o "$(MANUSCRIPT_JEKYLL)"
...
# Publish to a Minimal Mistakes blog repository. The post date and slug are
# fixed in helpers/publish_blog.py; each run stamps last_modified_at with
# today's date.
